...
#if 0
// Display a frame with resize and rotate support. It always fit full
// overlay window size. If application wants to change the window size
// or position, it must remap the buffer before using this function.
// IN		srcbuf, rotate
// srcbuf	source video buffer
// scale	1: scale, 2: no scale
// rotate	1: rotate 90L, 0: no rotate
//		rotate is valid only if scale=1
int display_write(int fd, struct pxa_video_buf* srcbuf,
		int scale, int rotate)
{
	int srcStep[3], dstStep[3];
	Ipp8u   *srcbuffer[3],*dstbuffer[3];
        IppiSize srcSize, dstSize;
	IppCameraRotation ipprotation;
	...
	ASSERT(rotate==1 || rotate==0);

	if(scale==0)return display_write_simple(fd,srcbuf);

	// Set buffer information (YCbCr422/YCbCr420)
	srcSize.width = srcbuf->width;
	srcSize.height = srcbuf->height;
	srcbuffer[0] = srcbuf->ycbcr.y;
	srcbuffer[1] = srcbuf->ycbcr.cb;
	srcbuffer[2] = srcbuf->ycbcr.cr;
	srcStep[0] = srcbuf->ycbcr.ystep;
	srcStep[1] = srcbuf->ycbcr.cbstep;
	srcStep[2] = srcbuf->ycbcr.crstep;

	dstbuffer[0] = devobj->vidbuf.ycbcr.y;
	dstbuffer[1] = devobj->vidbuf.ycbcr.cb;
	dstbuffer[2] = devobj->vidbuf.ycbcr.cr;
	dstStep[0] = devobj->vidbuf.ycbcr.ystep;
	dstStep[1] = devobj->vidbuf.ycbcr.cbstep;
	dstStep[2] = devobj->vidbuf.ycbcr.crstep;
	if(rotate){
		dstSize.width = devobj->vidbuf.height;
		dstSize.height = devobj->vidbuf.width;
	}else{
		dstSize.width = devobj->vidbuf.width;
		dstSize.height = devobj->vidbuf.height;
	}

	//DBGMSG("src.width=%d, src.height=%d, dst.width=%d, dst.height=%d\n",
	//	srcSize.width,srcSize.height,dstSize.width,dstSize.height);
	//DBGMSG("src.y=%x, src.cb=%x, src.cr=%x\n",
	//	srcbuffer[0],srcbuffer[1],srcbuffer[2]);
	//DBGMSG("src.ystep=%d, src.cbstep=%d, src.crstep=%d\n",
	//	srcStep[0],srcStep[1],srcStep[2]);
	//DBGMSG("dst.y=%x, dst.cb=%x, dst.cr=%x\n",
	//	dstbuffer[0],dstbuffer[1],dstbuffer[2]);
	//DBGMSG("dst.ystep=%d, dst.cbstep=%d, dst.crstep=%d\n",
	...
	ipprotation = rotate ? ippCameraRotate90L : ippCameraRotateDisable;
//	ipprotation = rotate ? ippCameraRotate90L : ippCameraRotate180;

	if(pxavid_ycbcr422==devobj->format){
		ret = ippiYCbCr422RszRot_8u_P3R(srcbuffer, 
				srcStep, srcSize, 
				dstbuffer, dstStep, dstSize,
		...
		//pvcount++;
		//if(fp==NULL&&pvcount==10){fp=fopen("vga.yuv","wb");
	...
	else if(pxavid_ycbcr420==devobj->format){
		ret = ippiYCbCr420RszRot_8u_P3R(srcbuffer, 
				srcStep, srcSize, 
				dstbuffer, dstStep, dstSize,
				ippCameraInterpBilinear,
				ipprotation,
